...
The model was developed by Jan Barendregt and colleagues (REF ACE prevention and core papers) and has been widely used in Australia and New Zeland (Ref ACE-report and BODE website of key publications). 
...
Same sources as above for population numbers. Sometimes mortality rates are in age groups (1-4, 5-9, etc). Interpolation can be used to derive in between ages rates (cubic spline). 
...
Each setting collection has a given number of diseases. Dismod II works with at least three of: case fatality, prevalence, incidence, mortality (disease), case fatality, remission, duration and the relative risk for mortality. So far, we have been assuming that remission is zero for chronic diseases, that is to say, when people become diseased, they do not recover. Special care should be taken with this assumption, as the GBD data assumes remission for some diseases, for example cancers, where after 10 years cases recover, exept for long term sequelae. Since GBD now provides prevalence, incidence and mortality, it may be best to use all three as Dismod II input parameters to compare the effect of the remission assumption by the GBD for some diseases.

#### Disability weights (quality of life weights) 

The PMSLT produces health-adjusted life years, this concept is similar to quality-adjusted life years (QALYs) and different to DALYs as those generated in the GBD as the sum of years of life lost (YLL) and years lived with disability (YLD). Health-adjusted life years, like QALYs, are a positive concept and are estimated by adjusting life years by the quality of life in those years. The quality of life compenent is derived from the rate of total years lived with disability (hence and input for the general life table as above described). The total rate of YLDs is then modified for the scanrios of interest by disease specific YLDS rates. To modify disease specific YLDs we need disability weights, and we derive these from prevalence and YLDs from the GBD studies (since YLDs=DW*P). This is 
4
